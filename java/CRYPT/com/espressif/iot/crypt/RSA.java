...
/*
 ...
 该算法于1977年由美国麻省理工学院MIT(Massachusetts Institute of Technology)的Ronal Rivest，Adi Shamir和Len Adleman三位年轻教授提出，并以三人的姓氏Rivest，Shamir和Adlernan命名为RSA算法，是�?��支持变长密钥的公共密钥算法，�?��加密的文件快的长度也是可变的!

 �?��RSA加密算法，是世界上第�?��非对称加密算法，也是数论的第�?��实际应用。它的算法如下：

 1.找两个非常大的质数p和q（�?常p和q都有155十进制位或都�?12十进制位）并计算n=pq，k=(p-1)(q-1)�?

 2.将明文编码成整数M，保证M不小�?但是小于n�?

 3.任取�?��整数e，保证e和k互质，�?且e不小�?但是小于k。加密钥匙（称作公钥）是(e, n)�?

 4.找到�?��整数d，使得ed除以k的余数是1（只要e和n满足上面条件，d肯定存在）�?解密钥匙（称作密钥）�?d, n)�?

 加密过程�?加密后的编码C等于M的e次方除以n�?��的余数�?

 解密过程�?解密后的编码N等于C的d次方除以n�?��的余数�?
...
public class RSA {
	...
	// Cipher cipher =
	// Cipher.getInstance(ConfigureEncryptAndDecrypt.RSA_ALGORITHM);
	...
	// public static boolean checkSign(String content, String sign, String
	// publicKey)
	// {
	// try
	// {
